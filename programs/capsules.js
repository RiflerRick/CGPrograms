...
function createCapsule( material, radius, top, bottom, segmentsWidth, openTop, openBottom )
{
	...
	object3D.add(cyl);
    //Remember that there are 3 steps to creating any object:
    /*
    create the material using MeshLambertMaterial(see doc for parameter list)
    create the geometry(various classes are there tobe used and each has its own set of parameter lists)
    create the Mesh using Mesh(see doc for parameter list)
    position the mesh correctly using the mesh object itself.
    */
    for (var i=0;i<2;i++)
	{
