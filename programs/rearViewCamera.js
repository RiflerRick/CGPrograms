...
function render() {
    ...
    /*
	Here the idea is to create a rear cam or simply a camera that will watch the behind when the front camera
	is watching the target itself.
	*/
	rearCam.position.copy(camera.position);
    //the above line enables the rear camera to always track the front camera.
    rearTarget.copy(camera.position);
    //.copy is a useful method that simply sets or copies the parameter inside the variable.
    //camera.position returns a vector3 object and rearTarget is also a vector3 object so this is a possible operation.
    rearTarget.sub(cameraControls.target);
    //this is essential because here we are subtracting cameraControls.target from camera.position and storing it in rearTarget
    //think of this as a vector going from the target towards the camera (the front or back camera)
    
    rearTarget.add(camera.position);
    //this is done because we also want the rearTarget to be a vector pointing towards the outside projecting out
    
    rearCam.lookAt(rearTarget);
    //finally we make the rearCam look towards the position of the rearTarget
